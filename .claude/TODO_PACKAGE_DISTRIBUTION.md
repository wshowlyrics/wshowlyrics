...
- [ ] **GPG 키 생성 및 관리**
  ...
  - [ ] **서명 대상**:
    ...
    - `PKGBUILD` for AUR stable (makepkg --sign) - 태그마다 재서명
    - `PKGBUILD` for AUR git (makepkg --sign) - 매 master 푸시마다 재서명
  - [ ] **체크섬 검증**:
    - AUR stable: `updpkgsums` 명령으로 sha256sums 자동 계산
    - AUR git: `sha256sums=('SKIP')` 유지 (Git 소스는 체크섬 불가능)
  - [ ] **서명 제외**:
...
- [ ] **`.github/workflows/release.yml` 수정**
  ...
  - [ ] AppImage 사용 방법 추가
  - [ ] 기존 AUR 로직 유지
  - [ ] **AUR stable 패키지 GPG 서명 및 체크섬 추가**:
    - [ ] GPG 키 import 단계 추가 (Phase 1과 동일)
    - [ ] 버전 업데이트: `sed -i "s/^pkgver=.*/pkgver=$VERSION/" PKGBUILD`
    - [ ] **체크섬 자동 계산**: `updpkgsums` (sha256sums 업데이트)
    - [ ] PKGBUILD 서명: `makepkg --sign`
    - [ ] `.SRCINFO` 재생성 (서명된 PKGBUILD 반영)
    - [ ] 서명 파일 커밋: `PKGBUILD`, `PKGBUILD.sig`, `.SRCINFO`
    - [ ] AUR에 푸시
    - [ ] **보안**: GitHub tarball 무결성 검증 (MITM 공격 방지)

  - [ ] **AUR git 패키지 GPG 서명 추가**:
    - [ ] **현재 상태**: `aur-publish.yml`이 매 master 푸시마다 자동 실행 ✅
      - pkgver 계산: `r$(git rev-list --count HEAD).$(git rev-parse --short HEAD)`
    ...
    - [ ] **추가 작업**: GPG 서명 통합
      ...
      - [ ] `makepkg --sign` 실행
      - [ ] `PKGBUILD.sig` 파일 커밋
      - [ ] 서명 파일 포함하여 AUR 푸시: `PKGBUILD`, `PKGBUILD.sig`, `.SRCINFO`
...
4. **최종 배포 전략**:
   - **Arch**:
     - Stable: AUR wshowlyrics (개발자 GPG 서명) ✅
     - Nightly: AUR wshowlyrics-git (개발자 GPG 서명) ✅
   - **Fedora**:
...
- ⚠️ 현재는 GPG 서명 없음 (Phase 3에서 추가 예정)
...
# AUR stable PKGBUILD 서명 확인
git clone https://aur.archlinux.org/wshowlyrics.git
cd wshowlyrics
makepkg -si  # 자동으로 PKGBUILD.sig 검증

# AUR git PKGBUILD 서명 확인
git clone https://aur.archlinux.org/wshowlyrics-git.git
cd wshowlyrics-git
makepkg -si  # 자동으로 PKGBUILD.sig 검증 → pkgver() 실행 → 최신 소스 다운로드
```
...
**패키지 서명 정책**:
- ✅ **개발자 GPG 서명**:
  - GitHub Releases: `.deb`, `.rpm`, `AppImage` (태그 릴리스)
  - AUR stable: `PKGBUILD.sig` (태그마다 재서명)
  - AUR git: `PKGBUILD.sig` (매 master 푸시마다 재서명) ⚠️ 추가 예정
...
**Nightly 빌드 제공 현황**:
- Arch: ✅ AUR wshowlyrics-git (⚠️ GPG 서명 추가 예정)

...
static bool handle_gemini_cache_loading(struct translation_thread_args *args,
                                         struct lyrics_data *data,
...
static bool process_gemini_line_translation(struct lyrics_line *line,
...
static void save_gemini_translation_to_cache(const char *cache_path,
                                              struct lyrics_data *data,
...
static void* translate_lyrics_async(void *arg) {
    ...
    if (!handle_gemini_cache_loading(args, data, translatable_count, &already_translated)) {
    ...
    while (line) {
        if (!process_gemini_line_translation(line, args, data, &current, translatable_count)) {
            break;  // Cancelled
        }
    ...
    save_gemini_translation_to_cache(args->cache_path, data, args->target_lang);

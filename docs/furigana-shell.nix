# nix-shell environment for docs/add_furigana.py and docs/add_furigana_ai.py.
...
#   nix-shell docs/furigana-shell.nix --run 'python docs/add_furigana_ai.py song.lrc'
# Add --regenerate to rebuild readings on already-annotated files:
#   nix-shell docs/furigana-shell.nix --run 'python docs/add_furigana.py --regenerate song.lrc'
{ pkgs ? import <nixpkgs> {} }:
pkgs.mkShell {
  buildInputs = [

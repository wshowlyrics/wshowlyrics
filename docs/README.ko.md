...
이 프로젝트는 가사 오버레이를 표시하기 위해 `wlr-layer-shell-unstable-v1` Wayland 프로토콜을 사용합니다.

### 공식 테스트된 환경

- **Sway** - 완벽하게 지원되며 테스트됨
- **Hyprland** - 완벽하게 지원되며 테스트됨
  - Hyprland의 내장 애니메이션 효과로 가사 오버레이 전환이 부드럽게 처리되어, 별도의 설정 없이도 자연스러운 시각적 경험을 제공합니다
- **KDE Plasma (5.27+)** - 완벽하게 지원되며 테스트됨

### 다른 컴포지터

wlr-layer-shell을 지원하는 다른 컴포지터(river, wayfire 등)에서도 작동할 수 있지만, 공식적으로 테스트되지 않았으며 안정성 문제가 있을 수 있습니다.

...
- **스마트 가사 검색**:
  ...
  - 파일명 기반 자동 매칭
- **카라오케 모드**: LRCX 포맷으로 단어별 타이밍 및 하이라이팅 지원
  - 지나간 단어: 일반 색상 (이미 불렀음)
  - 현재 단어: 하이라이트 색상 (지금 부르는 중)
...
| `-H COLOR` | `--highlight=COLOR` | 카라오케 하이라이트 색상 (#RRGGBB[AA] 형식) | `#87CEEBFF` (하늘색) |
...
# 커스텀 카라오케 하이라이트 색상
./build/lyrics -H FF1493FF  # 핑크색 하이라이트
./build/lyrics --highlight=00FF00FF  # 녹색 하이라이트
